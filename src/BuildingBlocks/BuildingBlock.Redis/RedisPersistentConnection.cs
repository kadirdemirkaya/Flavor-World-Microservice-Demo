...
namespace BuildingBlock.Redis
{
    public class RedisPersistentConnection : IDisposable
    {
        ...
        public RedisPersistentConnection(IConnectionMultiplexer redis, InMemoryConfig inMemoryConfig, int retryCount = 5)
        {
            connection = redis;
            RetryCount = retryCount;
            InMemoryConfig = inMemoryConfig;

            if (inMemoryConfig.Connection != null)
            {
                var connJson = JsonConvert.SerializeObject(inMemoryConfig.Connection, new JsonSerializerSettings()
                {
                    ReferenceLoopHandling = ReferenceLoopHandling.Ignore,
                });
                _conString = connJson;
